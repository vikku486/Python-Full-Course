"""
🙅‍♂️ Python Variables:- A variable in Python is a name that refers to a value stored in memory. You can think of it as a label for a container holding data.
👉 A variable is the name given to a memory location. A value-holding Python variable is also known as an identifier.
👉 Since Python is an interpreted and dynamically typed language, it automatically determines the typeof a variable at runtime, so we do not need to explicitly declare its type.
